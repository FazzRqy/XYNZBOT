...
export async function handler(conn, m, chatUpdate) {
  ...
  try {
    ...
    // if (m.message && m.message.stickerMessage && m.message.stickerMessage.fileSha256) {
    //   const buffer = Buffer.from(m.message.stickerMessage.fileSha256)
    //   const sha256Hash = crypto.createHash('sha256').update(buffer).digest('hex')
    //   const db_stickcmd = db.data.settings.sticker_cmd

    //   const foundStickerCmd = db_stickcmd.find(cmd => cmd.sticker_fileSha256 === sha256Hash)
      
    //   if (foundStickerCmd) {
    //     const cmd = foundStickerCmd.cmd

    //     const plugin = Object.values(global.plugins).find(plugin => 
    //       plugins.command && plugins.command.includes(cmd)
    //       )

    //   if (plugin && plugin.run && typeof plugin.run === 'function') {
    //     if (foundStickerCmd.query && foundStickerCmd.query.length > 0) {
    //       m.text = foundStickerCmd.query
    //     }
